...
#
    ...
    def realXPos(self,x1,y1,U1,V1, time):
        twoPi = 2*3.1415
        twoPi = 1.0
        theta = twoPi*time
        costheta = math.cos(theta)
    ...
    def returnC(self,x1,y1,U1,V1, time):
        
        x,y,vx,vy = self.realXPos(x1,y1,U1,V1, time)

        rho1 = math.sqrt( math.pow(x+self.mu1,2)  +y*y)
        rho2 = math.sqrt( math.pow(x-1.0+self.mu1,2)  +y*y)

        p1 = vx-y
        p2 = vy+x

        h = .5*(p1*p1+p2*p2) + p1*y - p2*x - (1-self.mu1)/rho1 -self.mu1/rho2
        return -2*h
    ...
    def step(self, step):
        ...
        x1Real, y1Real, U1Real, V1Real = self.realXPos(x1,y1,U1,V1, currentTime)

        self.xPosReal.append(x1Real)
        self.yPosReal.append(y1Real)
        self.xVelReal.append(U1Real)
        self.yVelReal.append(V1Real)

        #if(currentStep% 1000 == 0):
         #   print "----------------- energy: " ,self.returnC(x1,y1,U1,V1, currentTime)

        if (currentStep > 0):
            if (V1Real > 0 and self.yPosReal[currentStep]*self.yPosReal[currentStep-1] < 0):
                #print x1Real, y1Real, U1Real, V1Real
                self.xPosPoincare.append(x1Real)
                self.xVelPoincare.append(U1Real)
                print x1,y1,U1,V1, x1Real ,y1Real ,U1Real ,V1Real , currentTime
                print "----------------- energy: " ,self.returnC(x1,y1,U1,V1, currentTime)
    ...
    def returnXPosPoincare(self,skip):
        return self.xPosPoincare[::skip]


    def returnXVelPoincare(self,skip):
        return self.xVelPoincare[::skip]
...
#test = Asteriod(-.51,.88,-0.026,-0.015, False)
#test = Asteriod(-.63,-.92,0.058,0.0183, False) #~3.5

#test = Asteriod(-.53,.92,0.078,0.043, False)

#test = Asteriod(-.53,.92,0.078,0.043, False)

#test = Asteriod(.01,.09,.002,.001, False)
...
xPosPoincare = test.returnXPosPoincare(1)
xVelPoincare = test.returnXVelPoincare(1)
...
plt.scatter(test.xPos[::cut],test.yPos[::cut] , marker='.', c='r')
...
plt.savefig('output/xy_position_4+1_hw4.png'.format(totalTime))

plt.clf()


plt.scatter(xPosPoincare, xVelPoincare , marker='.', alpha=1.0, c='r',)
...
plt.savefig('output/poincare_plot_hw4_4_5plot.png'.format(totalTime))
...
plt.savefig('output/velocity_three_boyd_hw4.png'.format(totalTime))
...
plt.savefig('output/energy_three_boyd_hw4.png'.format(totalTime))
...
plt.savefig('output/dEnergy_three_boyd_hw4.png'.format(totalTime))



#plt.show()

#plt.savefig('output/xy_position_2_jupiter_zoom_bigger_1000.png'.format(totalTime))
